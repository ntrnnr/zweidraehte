...
//! STM32G0B0RE **KNX Data Secure** KNX-RF light switch with the **DoA
//! retransmitter** role (Semtech SX1211).
//!
//! Identical to
//! [`stm32g0_knxrf_secure_light_switch`](../stm32g0_knxrf_secure_light_switch)
//! — the same `devices::light_switch` definition over a Semtech SX1211, KNX
//! Data Secure, FRAM-backed sequence numbers — but it additionally composes
//! [`RfRetransmitterExtension`](zweidraehte_device::bcus::system_b::RfRetransmitterExtension)
//! and selects the
//! [`RetransmitEnabled`](zweidraehte_device::layers::linklayers::knxrf::RetransmitEnabled)
//! KNX-RF link-layer policy. So the device also acts as a Layer-2 DoA
//! retransmitter (repeater) per 03/02/05 §6.1.7: it re-broadcasts qualifying
//! frames on its Domain Address with the RF Repetition Counter decremented.
//!
//! The two opt-ins (`type ES` composing the extension, `type LLB` selecting
//! the policy) are checked against each other at compile time. The retransmitter
...
use zweidraehte_device::{
    bcus::system_b::*,
    config::MAX_APDU_LENGTH_RF,
    layers::linklayers::knxrf::{KnxRfLinkLayerBuilder, RetransmitEnabled},
    prelude::*,
...
type Stm32G0SecureState =
    SecureRfRetransmitterStateFor<Stm32G0KnxRfRetransmitter, Stm32G0SeqStorage, P2P_SIZE, SIAT_SIZE>;
...
type SecAugment<'a> =
    <<Stm32G0KnxRfRetransmitter as StackDefinition>::ES as zweidraehte_device::bcus::system_b::Extension<()>>::Augment<
        'a,
        Stm32G0KnxRfRetransmitter,
    >;
...
impl SystemBStackDefinition for Stm32G0KnxRfRetransmitter {}

impl HasSequenceStorage for Stm32G0KnxRfRetransmitter {
...
impl StackDefinition for Stm32G0KnxRfRetransmitter {
    ...
    // The `RetransmitEnabled` policy makes this the repeating KNX-RF link layer;
    ...
    type StateInit = SystemBStateInit<
        ...
        SecureResources<RfRetransmitterExtension, Stm32G0SeqStorage>,
    >;
...
async fn knx_task(runner: Runner<'static, Stm32G0KnxRfRetransmitter>) -> ! {
    runner.run().await
}

#[embassy_executor::task]
async fn prog_task(knx: Stack<'static, Stm32G0KnxRfRetransmitter>, prog_btn_pin: ExtiInput<'static>) -> ! {
...
#[embassy_executor::task]
async fn restart_task(knx: Stack<'static, Stm32G0KnxRfRetransmitter>, storage: &'static RefCell<Storage>) -> ! {
...
#[embassy_executor::task]
async fn lifecycle_task(knx: Stack<'static, Stm32G0KnxRfRetransmitter>) -> ! {
...
#[embassy_executor::task]
async fn app_task(knx: Stack<'static, Stm32G0KnxRfRetransmitter>, btn_pin: ExtiInput<'static>) -> ! {
...
#[embassy_executor::task]
async fn led_task(knx: Stack<'static, Stm32G0KnxRfRetransmitter>, mut led: Output<'static>) -> ! {
    let mut last = false;
    loop {
...
async fn main(spawner: Spawner) {
    ...
    // `.with_retransmitter()` selects the `RetransmitEnabled` policy to match
    // `type LLB`; the §6.1.7 repeating runs once PID 57 is enabled.
    let link_layer_builder =
        KnxRfLinkLayerBuilder::new(Sx1211Adapter::new(radio, pll_lock, threshold, irq0, data)).with_retransmitter();
        ...
        StackResources<
            Stm32G0KnxRfRetransmitter,
            {
                zweidraehte_device::config::buffer_size_for_apdu(
                    <Stm32G0KnxRfRetransmitter as StackDefinition>::MAX_APDU_LENGTH,
                )
            },
        >,
    > = StaticCell::new();
    ...
    let (knx_stack, knx_runner) = zweidraehte_device::new(
        ...
        state_init,
        (),
        Stm32G0KnxRfRetransmitter::memory_map(),
    );
