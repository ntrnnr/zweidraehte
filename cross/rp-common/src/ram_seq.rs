...
//! wear-levelled flash partition. (The RP2040 boards carry no FRAM, so
//! a production seq store on this platform means a wear-levelled flash
//! sector — see SESSION.md.)
//!
//! Byte-for-byte the same as `stm32-common`'s `RamSeqStorage`; it lives
//! here too because it has zero platform dependencies and the cross
//! workspaces don't share a crate.
//!
