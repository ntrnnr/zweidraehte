...
use net::PicoWIpTransport;
use network_info::PicoWNetworkInfo;
...
impl SystemBIpDeviceDef for PicoWLightSwitch {
    ...
    type Transport = PicoWIpTransport;
    type Platform = PicoWNetworkInfo;
...
impl StackDefinition for PicoWLightSwitch {
    ...
    type P = LightSwitchParams;
    type CO = LightSwitchComObjects;
    type LLB = KnxNetIpBuilder<PicoWIpTransport, 2>;
    type State = PicoWState;
...
async fn main(spawner: Spawner) {
    ...
    // Initialize the global network context so PicoWNetworkInfo::default()
    // works during device state construction (IpLinkLayerState::from_config
    // calls P::default()).
    PicoWNetworkInfo::init(stack, mac);
    ...
    let link_layer_builder =
        KnxNetIpBuilder::<PicoWIpTransport, 2>::new("wlan0", local_ip, control_endpoint, stack)
