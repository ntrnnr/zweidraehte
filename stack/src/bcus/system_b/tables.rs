...
//!
//! This module provides [`SystemBTables`], a container for the tables
//! required by a System B device.
...
/// Tables container for System B devices.
///
/// Contains the tables that are loaded via ETS:
///
...
/// # Persistence
///
/// Tables can be converted to/from [`PersistedState`] for storage.
/// Use [`from_persisted`](Self::from_persisted) to restore from storage,
/// and [`to_persisted`](Self::to_persisted) to prepare for saving.
///
/// Note: Configuration values (individual_address, auth_keys, ip_config) are
/// stored in [`DeviceState`](super::DeviceState) / [`IpDeviceState`](super::IpDeviceState),
/// not here. Pass them to `to_persisted()` when saving.
///
/// # Generic Parameters
///
...
pub struct SystemBTables<const ADT_SIZE: usize, const AST_SIZE: usize, const COT_SIZE: usize, P: ConstDefault = ()> {
    // ========================================================================
    // Tables (persisted via to_persisted/from_persisted)
    // ========================================================================
    ...
    /// Note: This is stored here for table building but the canonical
    /// source is DeviceState. When persisting, pass the value from state.
    routing_count: u8,
}

impl<const ADT_SIZE: usize, const AST_SIZE: usize, const COT_SIZE: usize, P: ConstDefault>
    SystemBTables<ADT_SIZE, AST_SIZE, COT_SIZE, P>
{
    /// Create new tables with factory defaults (unloaded).
    pub fn new() -> Self {
        Self {
    ...
    /// Create tables from persisted storage.
    ///
    /// Restores table data from storage.
    /// The application's run state is always set to `Halted` - it must
    /// be explicitly restarted after boot.
    ///
    /// # Arguments
    ///
    /// * `persisted` - The persisted state to restore from
    ...
    /// Export tables to persisted format for storage.
    ///
    /// Clones the current tables for persistence. You must provide the
    /// configuration values from [`DeviceState`](super::DeviceState).
    ///
    /// # Arguments
    ///
    /// * `individual_address` - Device individual address from DeviceState
    /// * `auth_keys` - Authorization keys from DeviceState
    /// * `ip_config` - IP configuration (for 57B0 devices) from IpDeviceState
    pub fn to_persisted(
        &self,
    ...
    ///
    /// Called when the routing count is changed via interface objects.
    pub fn set_routing_count(&mut self, count: u8) {
        self.routing_count = count;
    }
...
impl<const ADT_SIZE: usize, const AST_SIZE: usize, const COT_SIZE: usize, P: ConstDefault> Default
    for SystemBTables<ADT_SIZE, AST_SIZE, COT_SIZE, P>
{
    fn default() -> Self {
        Self::new()
    }
