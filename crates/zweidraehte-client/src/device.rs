...
impl DeviceConnection {
    pub(crate) fn new(remote: IndividualAddress, source: IndividualAddress, cmd_tx: CommandSender) -> Self {
        Self { remote, source, cmd_tx, send_seq: 0 }
    }
    ...
    async fn send_management_request(
        ...
        let cemi =
            transport::build_connected_data_cemi(self.source, self.remote, self.send_seq, apci, msg_len, data_writer);

        let expected_apci = management::expected_response_apci(apci);

        let (tx, rx) = oneshot::channel();
        self.cmd_tx
            .send(Command::SendFrame { cemi, expected_source: Some(self.remote), expected_apci, response_tx: tx })
            .await
    ...
    pub async fn property_read(&mut self, obj_idx: u8, prop_id: u16, start_idx: u16, count: u16) -> Result<Vec<u8>> {
        let buf = self
            .send_management_request(ApciCode::PropertyValueRead, PropertyValueHeader::MIN_MSG_LEN, |buf| {
    ...
    pub async fn property_write(
        ...
        let buf = self
            .send_management_request(ApciCode::PropertyValueWrite, PropertyValueResponse::msg_len(data.len()), |buf| {
    ...
    pub async fn property_description_read(
        ...
        let buf = self
            .send_management_request(ApciCode::PropertyDescriptionRead, PropertyDescriptionRead::MIN_MSG_LEN, |buf| {
    ...
    pub async fn memory_read(&mut self, address: u16, count: u8) -> Result<Vec<u8>> {
        let buf = self
            .send_management_request(ApciCode::MemoryRead, MemoryReadRequest::MSG_LEN, |buf| {
    ...
    pub async fn authorize(&mut self, key: &[u8; 4]) -> Result<u8> {
        let buf = self
            .send_management_request(ApciCode::AuthorizeRequest, AuthorizeRequest::MIN_MSG_LEN, |buf| {
                AuthorizeRequest::write(buf, key)
            })
            .await?;
