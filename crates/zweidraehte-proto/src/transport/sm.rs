//! Table-driven state machine for the connection-oriented transport layer
//!
...
fn execute_action<C: ConnectionCore>(action: SpecAction, conn: &mut C, ctx: &EventContext) -> ActionBuffer {
    ...
    match action {
        ...
        SpecAction::A1 => {
            ...
            conn.set_seq_no_send(0);
            conn.set_seq_no_recv(0);
            conn.set_rep_count(0);
        ...
        SpecAction::A8 => {
            ...
            // Queued outgoing data (from A11) is handled by the embedder
        ...
        SpecAction::A9 => {
            conn.set_rep_count(conn.rep_count() + 1);
...
#[cfg(test)]
mod tests {
    use super::super::connection_core::BasicConnection;
    use super::*;

    fn new_connection() -> BasicConnection {
        BasicConnection::new()
    }
    ...
    fn connect(conn: &mut BasicConnection, source: IndividualAddress, style: TlStyle) {
    ...
    fn send_data(conn: &mut BasicConnection, dest: IndividualAddress, style: TlStyle) {
    ...
    fn style1r_data_in_open_wait_queued() {
        ...
        // E04 (received data, correct seq) OPEN_WAIT → A2 in 1R.
