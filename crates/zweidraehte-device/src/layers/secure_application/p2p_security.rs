...
//!
//! Split out from the S-AL core so that 3.2's `HasP2pSecure` marker
//! trait can gate the entire P2P codepath at the type level.
//!
//! The S-A_Sync protocol is inherently P2P: it establishes sequence
//! number alignment between two individual peers. It requires either
//! the tool key (for ETS-initiated sync) or a P2P key from the P2P
//! Key Table (for device-to-device sync). Group-only devices that
//! don't configure a P2P Key Table still need to answer tool-key
//! sync requests during commissioning — that tool-key branch lives
//! here too, and is only dead when the marker gate in 3.2 removes
//! the whole file.
    ...
    <D::State as HasExtensionState>::ES: HasSecurityState,
{
    ...
    pub(super) fn process_sync_request(
        &self,
    ...
    pub(super) fn process_sync_response(
        &self,
