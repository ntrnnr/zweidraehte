...
fn handle_function_property_ext_command<D: StackDefinition>(
    ...
    //
    // ServiceIDs 0x01 (direct GroupValue_Write), 0x02 (transmit current GO
        ...
        && data.len() >= 2
    {
        ...
        let successful = match service_id {
            0x01 | 0x03 => result.return_code == 0x00,
            0x02 => result.return_code == 0x21,
            _ => false,
        };
        if successful {
            match service_id {
...
// ============================================================================
// GO Diagnostics Bus Telegram Helpers
// ============================================================================
...
fn handle_go_diag_direct_write_bus<D: StackDefinition>(data: &[u8], ctx: &AlExtensionContext<'_, D>) {
    ...
    let flags = data[2];
    let ga = u16::from_be_bytes([data[3], data[4]]);
    let value = &data[5..];

    // Bit 7 of flags: 1 = next full octet, 0 = 6 trailing bits after APCI.
    let full_octet = flags & 0x80 != 0;

    // Look up the TSAP for this group address.
    let tsap = ctx.state.adt().borrow().get_tsap(crate::address::GroupAddress(ga.to_be_bytes()));
    let Some(tsap) = tsap else {
        debug!("GO diag: no TSAP for GA 0x{:04X}", ga);
        return;
    };

    // Build the GroupValue_Write telegram. For short format (6 trailing bits),
    // the value goes in the low 6 bits of the APCI byte. For full octet
    // format, data starts after the APCI bytes.
    ...
    // TODO: Security flags (bits 0-1) control whether the telegram needs
    // S-frame wrapping. The S-AL wrapper handles this based on GO security
    // flags for the destination TSAP.
...
fn handle_go_diag_transmit_bus<D: StackDefinition>(data: &[u8], ctx: &AlExtensionContext<'_, D>) {
    ...
    let tsap = ctx.state.ast().borrow().get_sending_tsap(asap);
    let Some(tsap) = tsap else {
    ...
    let Some(cot_info) = cot.get_object(go_idx) else {
        return;
    };
    let (object_size, short_format) = cot_info.object_type.size_in_bytes();
    let priority = cot_info.flags.priority();
    drop(cot);
    ...
    let co = ctx.comm_objects.borrow();
