...
impl<'a, T: ComObjectValueType, const INDEX: u16> TypedComObj<'a, T, INDEX> {
    ...
    /// Get the 0-based logical (DSL) index for use with stack methods.
    /// The wire ASAP / ETS object number is this plus the family's
    /// `FIRST_ASAP` (0 for System 7, 1 for System B).
    #[inline]
...
/// Typed handle on a device's communication objects, generated by the
/// `EtsComObjects` derive. Both methods speak the 0-based logical (DSL)
/// index — the value of `#[ets(index = N)]`; the wire ASAP is
/// `logical + FIRST_ASAP`.
...
pub trait ComObjects {
    ...
    /// Logical index of the device's lowest-numbered communication
    /// object. The `EtsComObjects` derive sets it to the smallest
    /// `#[ets(index = N)]`; the default covers containers with no
    /// objects. The AL's read-on-init self-detection probes this
    /// object's status.
