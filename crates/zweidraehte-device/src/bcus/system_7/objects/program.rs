...
//! - **Application Program 2** (Object Type 4, index 4) — the type the
//!   spec calls "Interfaceprogram" (Annex A.2.7); on System 7 it is a
//!   second application program, not a PEI, but the wire object type is
//!   the same 0004h.
//!
//! Differences from the System B objects they mirror:
//!
//! - 16-level access levels (Annex A.2.6/A.2.7, 0705h column):
//!   everyone-readable = 15, state machines writable from level 1.
...
system_7_program_object!(
    /// Application Program 2 Object (Object Type 4) for System 7.
    ///
    /// Uses the second-program [`RunTarget::Pei`] role internally; the
    /// wire type is `InterfaceProgram` (0004h) per Annex A.2.7.
