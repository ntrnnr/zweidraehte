...
/// The standard device-model implementation, shared by the BCU families.
///
...
///
/// Everything it touches goes through the generic trait surface
/// (`HasApplication`, `HasCommObjects`, the interface-object container's
/// `set_user_stopped`), so a BCU family only needs its own device model
/// when its lifecycle side effects differ — the families themselves alias
/// this type (`SystemBDeviceModel`).
///
/// The second program object ([`RunTarget::Pei`]) carries no
/// `user_stopped` flag and no comm objects; only its lifecycle events are
/// surfaced. System B's PEI Program Object and System 7's Application
/// Program 2 both fill that role.
pub struct StandardDeviceModel<'a, D: StackDefinition> {
    state: &'a D::State,
    layer_context: &'a LayerContext<D>,
    interface_objects: &'a D::InterfaceObjects<'static>,
}

impl<'a, D: StackDefinition> StandardDeviceModel<'a, D> {
    pub fn new(
        state: &'a D::State,
        layer_context: &'a LayerContext<D>,
        interface_objects: &'a D::InterfaceObjects<'static>,
...
impl<D: StackDefinition> DeviceModel for StandardDeviceModel<'_, D> {
    fn init(&mut self) {
...
impl<D: StackDefinition> LifecycleHook<D> for StandardDeviceModel<'_, D> {
    fn init(&mut self) {
...
impl<D: StackDefinition> StandardDeviceModel<'_, D> {
    fn on_action_for(&mut self, target: RunTarget, action: RunAction) {
        match (target, action) {
            ...
            // The second program object has no `user_stopped` flag and no
            // associated comm objects — only the lifecycle event is surfaced.
