...
//! reference implementations don't read yet; they decrypt and parse like
//! the rest and are retained for client-side commissioning.
...
pub enum KnxKeysError {
    ...
    #[error("keyring XML is malformed")]
    Xml(#[from] quick_xml::Error),

    #[error("keyring XML contains a malformed attribute")]
    XmlAttribute(#[from] quick_xml::events::attributes::AttrError),

    #[error("keyring XML contains an invalid escaped value")]
    XmlEscape(#[from] quick_xml::escape::EscapeError),

    #[error("keyring XML has an invalid structure: {0}")]
    ...
    #[error("the operating system random source failed during keyring export: {0}")]
    Random(getrandom::Error),

    #[error("a keyring password value is too large to encrypt")]
    PasswordTooLarge,
}
...
fn encrypt_password(value: &str, key: &[u8; 16], iv: &[u8; 16]) -> Result<String, KnxKeysError> {
    ...
    getrandom::fill(&mut plaintext[..8]).map_err(KnxKeysError::Random)?;
    ...
    let encrypted = Aes128CbcEnc::new(key.into(), iv.into())
        ...
        .map_err(|_| KnxKeysError::PasswordTooLarge)?;
        ...
        match el.local_name().as_ref() {
            ...
            b"Group" => {
                ...
                match section {
                    ...
                    _ => return Err(KnxKeysError::XmlStructure("Group element outside any container")),
                }
            }
