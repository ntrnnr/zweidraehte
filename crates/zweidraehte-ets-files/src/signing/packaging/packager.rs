...
use super::{ConverterKey, SigningConfig};
use crate::signing::master_data::get_master_data;
use crate::signing::{MasterDataSource, SigningError};

type DirectoryFiles = Vec<(String, Vec<u8>)>;
type SignedDirectory = (DirectoryFiles, String);
...
pub fn sign_hardware_xml(
    ...
    loop {
        match reader.read_event() {
                ...
                match name {
                    ...
                    "Product" if in_products => {
                        ...
                        if let Some(ref hw_attrs) = current_hardware_attrs {
                            ...
                            let signature = create_registration_signature(&hash_bytes, "Registered", None, key)?;
                ...
                match name {
                    "Product" if in_products => {
                        ...
                        if let Some(ref hw_attrs) = current_hardware_attrs {
                            ...
                            writer.write_event(Event::Empty(reg_elem))?;
            ...
            Ok(Event::Eof) => break,
            Err(error) => return Err(SigningError::XmlRead(error)),
        }
    }

    let result = writer.into_inner().into_inner();
    String::from_utf8(result).map_err(SigningError::Utf8)
}
...
fn sign_manufacturer_files(
    ...
    key: &ConverterKey,
) -> Result<SignedDirectory, SigningError> {
    ...
    let signed_hardware = sign_hardware_xml(&config.hardware, &all_app_hashes, key)?;
    ...
    let dir_signature = sign_directory_contents(&files_refs, key)?;
...
pub fn create_knxprod(
    ...
    let (dir_files, dir_signature) = sign_manufacturer_files(config, true, key)?;
...
pub fn create_knxproj(
    ...
    create_knxproj_multi(std::slice::from_ref(config), project, master_data, key)
}

/// Create a signed `.knxproj` containing any number of manufacturer
/// directories. Each directory is signed independently, matching ETS's
/// archive layout.
pub fn create_knxproj_multi(
    configs: &[SigningConfig],
    project: &ProjectConfig,
    master_data: MasterDataSource,
    key: &ConverterKey,
) -> Result<Vec<u8>, SigningError> {
    ...
    let mut manufacturers = Vec::with_capacity(configs.len());
    for config in configs {
        let directory = format!("M-{}", config.manufacturer_id.strip_prefix("M-").unwrap_or(&config.manufacturer_id));
        if !directories.insert(directory.clone()) {
            return Err(SigningError::DuplicateManufacturer(directory));
        }
        let (files, signature) = sign_manufacturer_files(config, false, key)?;
        manufacturers.push((directory, files, signature));
    }
    ...
    let project_signature = sign_directory_contents(&project_refs, key)?;
    ...
    let mut zip_buffer = Cursor::new(Vec::new());
    {
        ...
        for (directory, files, signature) in &manufacturers {
            write_signed_directory(&mut zip, directory, files, signature, file_options)?;
        }
