...
use super::keys::{ConverterKey, KeyType, get_converter_public_key, get_knx_cert_public_key};
use super::{
...
fn sign_sha1(data: &[u8], key: &ConverterKey) -> Result<Vec<u8>, SigningError> {
    // Hash with SHA1
...
pub fn create_registration_signature(
    ...
    let signature = sign_sha1(&message, key)?;
...
pub fn verify_directory_signature(
    ...
    let signature = match BASE64.decode(signature_b64.trim_start_matches('\u{feff}').trim()) {
        Ok(s) => s,
        Err(e) => {
    ...
    // dragged the whole icu crate tree into `signing`. RSA verification
...
pub fn sign_directory_contents(files: &[(String, &[u8])], key: &ConverterKey) -> Result<String, SigningError> {
    ...
    let digest = build_digest_string(files);
    let signature = sign_sha1(digest.as_bytes(), key)?;
    Ok(BASE64.encode(&signature))
}
...
mod tests {
    ...
    fn converter_key() -> ConverterKey {
        let path = std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("../..").join("converter_key.xml");
        ConverterKey::from_file(path).expect("converter key loads")
    }
    ...
    fn test_sign_and_verify_registration() {
        ...
        let signature =
            create_registration_signature(&parent_hash, "Registered", None, &converter_key()).expect("sign");
    ...
    fn test_sign_directory() {
        ...
        let signature = sign_directory_contents(&files, &converter_key()).expect("sign");
        assert!(BASE64.decode(&signature).is_ok());
        ...
        let result = verify_directory_signature(&files, &format!("\u{feff}{signature}"))
            .expect("BOM-prefixed ETS signature verifies");
        assert!(result.valid);
    }
    ...
    fn signing_rejects_paths_outside_the_validated_charset() {
        ...
        let err =
            sign_directory_contents(&files, &converter_key()).expect_err("non-ASCII letters must not be signable");
        ...
        let err = sign_directory_contents(&files, &converter_key())
            .expect_err("unvalidated punctuation must not be signable");
