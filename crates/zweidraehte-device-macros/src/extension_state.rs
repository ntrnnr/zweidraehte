...
fn gen_extension_state_impl(
    ...
    // `apply_config` writes through `&self`, which needs interior
    // mutability on every persisted field. A plain persisted field is
    // legal for `from_config` (set once at construction), so rather than
    // erroring we simply skip generating `apply_config` in that case.
    let mut all_fields_apply_through_ref = true;
    ...
    for model in models {
        match model {
            FieldModel::Persisted(p) => {
                ...
                match &p.wrapper {
                    Wrapper::Cell(_) => apply_stmts.push(quote! { self.#ident.set(#inner_from); }),
                    Wrapper::RefCell(_) => apply_stmts.push(quote! { *self.#ident.borrow_mut() = #inner_from; }),
                    Wrapper::Plain(_) => all_fields_apply_through_ref = false,
                }
    ...
    // `apply_config` is the in-place sibling of `from_config`: it writes a
    // config snapshot through `&self` via the fields' interior mutability,
    // leaving `#[runtime_only]` fields untouched. Hand-written
    // `on_erase = manual` impls use it to collapse the per-field reset
    // copy (`self.apply_config(Config::default())` + side-effects) so a
    // new persisted field can't be forgotten in the erase path.
    let apply_config_impl = if all_fields_apply_through_ref {
        quote! {
            impl #impl_generics #state_ident #ty_generics #where_clause {
                /// Apply a persisted config snapshot in place.
                ///
                /// Writes every persisted field through its `Cell`/`RefCell`;
                /// runtime-only fields are untouched. Generated by
                /// `#[derive(ExtensionState)]`.
                pub fn apply_config(&self, config: #config_ident) {
                    #( #apply_stmts )*
                }
            }
        }
