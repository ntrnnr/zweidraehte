...
/// on every method). The visibility does not leak out of `generator`.
pub struct KnxprodBuilder<'a> {
    pub(super) manufacturer_id: u16,
    pub(super) application_programs: Vec<&'a ApplicationProgramDef<'a>>,
    pub(super) hardware_defs: Vec<HardwareDef<'a>>,
    pub(super) catalog_sections: Vec<CatalogSectionDef<'a>>,

    // Name used for the knxprod filename (derived from first app program).
    pub(super) knxprod_name: Option<String>,
...
impl<'a> KnxprodBuilder<'a> {
    ...
    pub fn single_device(def: SingleDeviceDef<'a>) -> Self {
        ...
        let hw_ref = builder.hardware(HardwareDef {
            ...
            is_rf_retransmitter: def.is_rf_retransmitter,
            rf_rx_capabilities: def.rf_rx_capabilities,
            rf_tx_capabilities: def.rf_tx_capabilities,
