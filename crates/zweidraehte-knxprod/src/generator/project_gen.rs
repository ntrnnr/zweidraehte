...
//!
//! Generates the two project-level XML files that distinguish a
//! `.knxproj` from a plain `.knxprod`:
//!
//! - `project.xml` — project metadata (name, GUID, Puid counter)
//! - `0.xml` — topology with device instances placed in
//!   `UnassignedDevices`
//!
//! Packaging-only, and gated once at its declaration in [`super`]; the
//! [`DeviceInstanceDef`](super::project::DeviceInstanceDef) input type
//! it consumes stays unconditional.
...
use super::project::DeviceInstanceDef;
...
// The generator itself is only driven by the `.knxproj` packaging path
// (`KnxprodBuilder::build_knxproj`); `DeviceInstanceDef` above stays
// unconditional because instances are registered on the builder before
