...
impl MtxmlGenerator {
    ...
    fn build_knx(
        ...
        if let Some(translations) = config.translations {
            ...
            // and from module dynamic sections.
            let mut block_name_map = app.dynamic.as_ref().map(collect_block_name_map).unwrap_or_default();

            // Also collect from module dynamic sections
            if let Some(ref module_defs) = app.module_defs {
                for md in &module_defs.module_defs {
                    if let Some(ref dyn_section) = md.dynamic {
                        collect_module_block_name_map(dyn_section, &mut block_name_map);
                    }
                }
            }
            ...
            let obj_ref_id_map: HashMap<String, String> = app
                .static_section
                .com_object_refs
                .as_ref()
                .map(|xml_refs| {
                ...
                if let Some(ref params) = app.static_section.parameters {
                    for item in &params.items {
                        if let ParameterItem::Union(union_elem) = item {
                            ...
                            let field_name =
                                union_elem.parameters.first().and_then(|p| p.name.strip_suffix("_selector"));
        ...
        knx.manufacturer_data.manufacturer.application_programs.programs.push(app);
    ...
    fn build_languages(
        ...
        let mut by_language: BTreeMap<&str, Vec<&EtsTranslation>> = BTreeMap::new();
        ...
        for (lang_id, trans_list) in by_language {
            ...
            let mut by_ref_path: BTreeMap<&str, Vec<&EtsTranslation>> = BTreeMap::new();
    ...
    fn translation_ref_path_to_ids(
        ...
        if let Some(obj_name) = ref_path.strip_prefix("obj::") {
            ...
            // matching the IDs generated by build_module_com_objects.
            if let Some(ref modules) = config.modules {
                for (def_idx, module_def) in modules.definitions().iter().enumerate() {
                    if let Some(objs) = module_def.comm_objects
                        && let Some(obj) = objs.iter().find(|o| o.name == obj_name)
                    {
                        let module_id = format!("{}_MD-{}", app_id, def_idx + 1);
                        return Ok(vec![format!("{}_O-2-{}", module_id, obj.index)]);
                    }
                }
            }
            // Object not found
        ...
        if let Some(param_name) = ref_path.strip_prefix("param::") {
            ...
            // Module parameter IDs use the format {app_id}_MD-{N}_P-{param_num},
            // matching the IDs generated by build_module_parameters.
            if let Some(ref modules) = config.modules {
                for (def_idx, module_def) in modules.definitions().iter().enumerate() {
                    if let Some(param_num) = module_def.find_param_num_by_name(param_name) {
                        let module_id = format!("{}_MD-{}", app_id, def_idx + 1);
                        return Ok(vec![format!("{}_P-{}", module_id, param_num)]);
                    }
                }
            }
            // Parameter not found
            ...
            for union_field in union_fields.iter() {
                ...
                for variant_param in union_field.union_info.variant_params.iter() {
                    if let Some(variants) = variant_param.enum_variants
                        && type_matches(variant_param.param.type_name)
                    {
                        for variant in variants.iter() {
                            if variant_matches(variant) {
                                return Some(variant.value);
    ...
    fn build_static_section(
        ...
        let (parameter_refs, param_ref_ids) = Self::build_parameter_refs(config, app_id, union_variant_texts.as_ref());
        ...
        Ok((
            StaticSection {
                ...
                extension: Some(Extension {
                    baggages: config
                        .baggages
                        .map_or(Vec::new(), |b| baggages_to_refs(config.device.manufacturer_id, b)),
                }),
                messages: None,
                bus_interfaces: config.bus_interfaces.map(|defs| BusInterfaces {
                    interfaces: defs
                        .iter()
                        .enumerate()
            ...
            let param_size: u32 = def
                ...
                .map(|p| {
                    p.iter()
                        ...
                        .map(|param| (param.base.size_bits as u32).div_ceil(8))
                        .sum()
                })
                .unwrap_or(0);
    ...
    fn build_module_dynamic_from_layout(
        ...
        if dynamic_items.is_empty() { None } else { Some(ModuleDefDynamic { items: dynamic_items }) }
    }
            ...
            match item {
                ...
                ModuleLayoutItem::Obj(name) => {
                    ...
                    if let Some(objs) = def.comm_objects
                        && let Some(idx) = objs.iter().position(|o| o.name == *name)
                    {
                        let ref_num = idx + 1;
    ...
    fn build_default_module_dynamic(
        ...
        let has_params = def.params.is_some_and(|p| !p.is_empty());
        let has_comm_objs = def.comm_objects.is_some_and(|c| !c.is_empty());
    ...
    fn build_parameters(config: &ApplicationProgramConfig, app_id: &str, code_segment_id: &str) -> Parameters {
        ...
        for param_ext in config.all_params() {
            ...
            let size_bytes = (param.size_bits as usize).div_ceil(8);
    ...
    fn build_union(
        ...
        for param in union_info.variant_params {
            ...
            let default_value = if let Some(val) = param.default_value {
                val.to_string()
            } else if param.param.text_pattern.is_some_and(|p| p.contains("TypeColor")) {
                "#000000".to_string()
            } else {
                "0".to_string()
            };
    ...
    fn resolve_text_param_ref_v20(text: &str, param_ref_ids: &BTreeMap<String, String>) -> (String, Option<String>) {
        if !text.contains("{{") {
    ...
    fn build_dynamic_section(
        ...
        if let Some(union_fields) = config.union_fields {
            ...
            for field in union_fields {
                ...
                for (discriminant, (_display_name, param_ref_ids)) in sorted_variants {
                    ...
                    let when_items: Vec<WhenItem> = param_ref_ids.into_iter().map(when_param_ref).collect();
        ...
        let co_ref_offset = {
            ...
            let union_count = config
                .union_fields
                .map(|fields| fields.iter().map(|f| 1 + f.union_info.variant_params.len() as u32).sum::<u32>())
                .unwrap_or(0);
        ...
        for (selector_param, objects) in &selector_groups {
            ...
            let whens: Vec<When> = sorted_values
                ...
                .map(|(selector_value, ref_ids)| {
                    let when_items: Vec<WhenItem> = ref_ids.into_iter().map(when_com_obj_ref).collect();
    ...
    fn build_dynamic_section_from_layout(
        ...
        let param_ref_map = Self::build_param_ref_map(config, app_id, Some(&union_variant_texts));
    ...
    fn build_inline_union_selector_block(
        ...
        *block_counter += 1;
        ...
        if let Some(union_fields) = config.union_fields
            && let Some(union_info) = union_fields.iter().find(|u| u.field_name == union_name)
        {
                ...
                PageItem::Obj(name) => {
                    ...
                    if let Some(refs) = comm_obj_ref_map.get(*name) {
                        ...
                        if refs_with_selector.is_empty() {
                            ...
                            let mut by_selector: BTreeMap<&str, Vec<(&String, i64)>> = BTreeMap::new();
                            ...
                            for (selector_param, ref_vals) in by_selector {
                                ...
                                if let Some(active_vals) = active_conditions.get_active_values(selector_param) {
                                    ...
                                    let mut value_to_ref: BTreeMap<i64, &String> = BTreeMap::new();
                            ...
                            if let Some((sub_selector_param, sub_variants)) = sub_selector_map.get(&selector_value) {
                                ...
                                for (sub_value, ref_name, variant_name) in *sub_variants {
                                    ...
                                    if let Some(named_refs) = comm_obj_ref_map.get(*ref_name)
                                        && let Some((ref_id, _, _)) = named_refs.first()
                                    {
        ...
        if let Some(params) = &app.static_section.parameters {
            for item in &params.items {
                if let ParameterItem::Parameter(param) = item
                    && !param_type_ids.contains(param.parameter_type.as_str())
                {
...
/// Collect block name → XML ID mappings from a module's dynamic section.
///
/// Module dynamic sections use the same `ParameterBlock` and `Choose` types
/// as the top-level dynamic section, but are wrapped in `ModuleDefDynamic`.
fn collect_module_block_name_map(dynamic: &ModuleDefDynamic, map: &mut HashMap<String, String>) {
    fn collect_from_block(block: &ParameterBlock, map: &mut HashMap<String, String>) {
        if let Some(ref name) = block.name {
            map.insert(name.clone(), block.id.clone());
        }
    ...
    for item in &dynamic.items {
        match item {
            ModuleDefDynamicItem::ParameterBlock(block) => collect_from_block(block, map),
