...
use zweidraehte::{
    layers::{
        ActorRequest, Layer, LayerOp,
        linklayers::knxip::{EndpointType, KnxNetIp, KnxNetIpBuilder, servers},
    },
    messages::{
...
// Helper function to run the link layer without task annotation (since we can't use generics with tasks)
async fn run_link_layer_impl<const N_SERVERS: usize, const N_REGISTRATIONS: usize>(
    mut ll: KnxNetIp<N_SERVERS, N_REGISTRATIONS>,
    link_receiver: Receiver<'static, NoopRawMutex, LayerOp<KnxMessageBuffer<Buffer<'static>>>, 32>,
) -> ! {
...
async fn main(spawner: Spawner) {
    ...
    let _network_sender: DynamicSender<'_, LayerOp<KnxMessageBuffer<Buffer<'static>>>> =
        network_channel.sender().into();
    ...
    // Discovery has 5 interests + Routing has 3 interests = 8 total registrations
    // Endpoints are automatically sized to match registrations (max possible unique endpoints)
    let k = kb.add_server(ds).add_server(rs).add_server(cs).build();
    println!("Created KNXnet/IP layer: {:#?}", k);
    ...
    // Run the link layer and test loop concurrently
    let link_layer_future = run_link_layer_impl(k, link_receiver);
