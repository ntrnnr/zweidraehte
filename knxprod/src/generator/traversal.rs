...
// ============================================================================
// Selector Usage Counting
// ============================================================================

/// Visitor that counts selector usages.
struct SelectorCounter<'a> {
    counts: &'a mut HashMap<String, usize>,
    comm_obj_ref_map: &'a HashMap<String, Vec<(String, Option<String>, Option<i64>)>>,
}

impl PageLayoutVisitor for SelectorCounter<'_> {
    fn visit_obj_with_value(
        &mut self,
        _obj_name: &str,
        selector_param: &str,
    ...
    fn visit_obj(&mut self, name: &str) {
        // Obj items also create choose blocks - count selectors from the object refs
        if let Some(refs) = self.comm_obj_ref_map.get(name) {
            ...
            for (_, sel_param, sel_val) in refs {
                if let (Some(param), Some(_)) = (sel_param.as_ref(), sel_val) {
                    if seen_selectors.insert(param.as_str()) {
                        *self.counts.entry(param.clone()).or_insert(0) += 1;
                    }
                }
            }
        }
...
/// Counts how many times each parameter is used as a selector.
pub(crate) fn count_selector_usages_with_objects(
