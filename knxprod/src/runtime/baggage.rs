...
impl std::fmt::Display for BaggageError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::Io(e, path) => write!(f, "Failed to read {}: {}", path.display(), e),
            Self::Parse(msg) => write!(f, "Failed to parse Baggages.xml: {}", msg),
        }
    }
}
...
// XML deserialization types for Baggages.xml
...
struct BaggagesManufacturer {
    ...
    baggages: Option<BaggagesList>,
}

#[derive(Debug, Deserialize)]
struct BaggagesList {
    #[serde(rename = "Baggage", default)]
    items: Vec<BaggageXml>,
}
