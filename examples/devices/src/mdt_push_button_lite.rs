...
pub struct MdtParams {
    ...
    /// Filler for the alignment hole at offset 3: `long_action_time` is 2-byte
    /// aligned and `debounce_time` ends on an odd offset. Naming the byte keeps
    /// the blob deterministic instead of leaking stack contents.
    #[ets(skip)]
    #[serde(skip)]
    pub _pad_3: u8,
    ...
    /// Filler for the alignment hole at offset 435 — see [`MdtParams::_pad_3`].
    #[ets(skip)]
    #[serde(skip)]
    pub _pad_435: u8,
    ...
    /// Filler for the trailing hole at offset 547: the struct is 2-byte aligned,
    /// so its size rounds up past the last 1-byte field. See [`MdtParams::_pad_3`].
    #[ets(skip)]
    #[serde(skip)]
    pub _pad_547: u8,
}
