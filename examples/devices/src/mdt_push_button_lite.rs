...
use zweidraehte_ets_model::{EtsEnum, ets_com_objects, ets_params, ets_union};
...
/// MaskVersion: MV-0705 (System 7 TP1)
...
/// Device metadata anchor for the MDT replication.
///
/// Carries the [`EtsPageLayout`] impl. There is intentionally no
/// `StackDefinition` for this device: the replication exists to prove ETS
/// DSL parity with the vendor XML, and only the generator consumes it.
#[derive(Debug, Clone, Copy)]
pub struct MdtPushButtonLiteDevice;
...
pub enum ButtonValueUnion {
    ...
    #[ets(skip)]
    _Reserved {
        /// Anchor byte count — widens the union's data area beyond what any
        /// real variant needs, so the ETS layout matches the vendor program.
        _anchor: [u8; 4],
    },
}
...
pub mod comm_objs {
    ...
    pub struct MdtComObjects {
        ...
        pub button1_secondary: DPT_Switch,
        ...
        pub button1_status_toggle: DPT_Switch,
        ...
        pub button1_status_display: DPT_Switch,
        ...
        pub button1_extra_long: DPT_Switch,
        ...
        pub button1_blocking: DPT_Enable,
        ...
        pub button2_secondary: DPT_Switch,
        ...
        pub button2_status_toggle: DPT_Switch,
        ...
        pub button2_status_display: DPT_Switch,
        ...
        pub button2_extra_long: DPT_Switch,
        ...
        pub button2_blocking: DPT_Enable,
        ...
        #[ets_ref(dpt = DPT_Colour_RGB, when = ObjectType::Rgb, function = "RGB value")]
        #[ets_ref(dpt = DPT_Colour_RGB, when = ObjectType::Rgb, function = "HSV value")]
        pub slap_short_main: DPT_Switch,
        ...
        #[ets_ref(dpt = DPT_Switch, when = ObjectType::Switch, function = "Status for toggle", transmit = true, update = true)]
        pub slap_short_status: DPT_Switch,
        ...
        #[ets_ref(dpt = DPT_Switch, when = ObjectType::Switch, function = "Status for toggle", transmit = true, update = true)]
        #[ets_ref(dpt = DPT_Switch, when = ObjectType::Switch, function = "Status for toggle", transmit = true, update = true)]
        pub slap_long_status: DPT_Switch,
...
///
/// The generator reinterprets this struct as the `<Data>` defaults blob ETS
/// reads back byte-for-byte, so every byte — including ones no field claims —
/// lands at a real parameter offset in the product database.
///
/// Hence the derived [`IntoBytes`]: zerocopy rejects a struct with padding,
/// pinning the blob to a deterministic image. The `_pad_*` fields below name
/// the alignment holes that check would otherwise reject. Their offsets are not
/// arbitrary — each sits exactly where the compiler was already padding, so
/// adding them changed no other field's offset and left the blob's size at 548.
/// If you reorder or resize fields, re-derive the hole positions; `IntoBytes`
/// refuses to compile if you get it wrong, which is the point.
#[ets_params]
#[derive(Debug, Clone, Copy, Serialize, Deserialize)]
...
// Default and ConstDefault are auto-generated by #[ets_params]
// based on #[ets(default = X)] attributes on fields and ConstDefault impls
// for ets_enum and union fields.

// NOTE: `MdtParams` no longer carries a hand-written `unsafe impl IntoBytes`.
// The old one conceded the struct had padding and argued the bytes were still
// zero "because ConstDefault::DEFAULT is a const". That did not hold: the
// generator built the blob from the *runtime* `Default::default()`, which
// materialises the struct field by field on the stack and never writes the
// holes, so they kept whatever the stack held — the source of the defaults
// changing between builds of identical source.
//
// It also claimed no-padding was "not achievable without reordering all fields
// or inserting explicit padding fields, which would break ETS layout
// compatibility". Fillers placed at the offsets the compiler was already
// padding change no other field's offset; the blob is still 548 bytes.

// ============================================================================
// ETS Page Layout
// ============================================================================

impl EtsPageLayout for MdtPushButtonLiteDevice {
...
mod tests {
    ...
    fn test_button_value_union_pattern_matching() {
        ...
        params.button1_value_00 = ButtonValueUnion::percent(Select0to100Percent::P25);
        match params.button1_value_00 {
        ...
        params.button1_value_00 = ButtonValueUnion::colour_temp(2700);
        match params.button1_value_00 {
        ...
        // Set to RGB variant
        params.button1_value_00 = ButtonValueUnion::rgb([255, 128, 0]);
        match params.button1_value_00 {
    ...
    fn test_button_press_writes_to_comm_object() {
        ...
        match params.button1_value_00 {
            ...
            ButtonValueUnion::_Reserved { .. } => unreachable!(),
        }
    ...
    fn test_time_duration_union_access() {
        ...
        params.button1_time_duration = TimeDurationUnion::long_keypress_time(TimeForLongKeypress::S1);
        match params.button1_time_duration {
            TimeDurationUnion::LongKeypressTime { value, .. } => {
    ...
    fn test_exhaustive_matching_all_button_value_variants() {
        let variants = [
            ButtonValueUnion::switch(GedptSwitch::On),
            ButtonValueUnion::forcible_control(Zwangsfuehrung::PriorityOn),
            ButtonValueUnion::percent(Select0to100Percent::P50),
            ButtonValueUnion::decimal(42),
            ButtonValueUnion::scene(SceneValue::Scene1),
            ButtonValueUnion::colour_temp(4000),
            ButtonValueUnion::temperature(0x0C1A),
            ButtonValueUnion::brightness(0x1F00),
            ButtonValueUnion::rgb([255, 0, 128]),
            ButtonValueUnion::switch1_bit(GedptSwitch::Off),
            ButtonValueUnion::hsv([180, 255, 200]),
        ];
