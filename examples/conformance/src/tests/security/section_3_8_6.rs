...
const PLAIN_READ_PID47: &str = "BC #EDI #BDUT_ADDR 69 01 CC 00 00 00 10 47 02 00 01";

// Plain error response: count=0, return_code=0xFC (E_ACCESS_DENIED).
// APDU: 01 CD + 00 00 + 00 10 + 47 + 00 + 00 01 + FC = 11 bytes → TP1 len = 0x6A
const PLAIN_READ_PID47_DENIED: &str = "BC #BDUT_ADDR #EDI 6A 01 CD 00 00 00 10 47 00 00 01 FC";
...
const PLAIN_READ_PID47_OK: &str = "BC #BDUT_ADDR #EDI 6D 01 CD 00 00 00 10 47 02 00 01 00 00 ?? ??";

// Secure read template (same inner APDU, carried in extended frame for secure wrapping).
const SECURE_READ_PID47: &str = "3C 60 #EDI #BDUT_ADDR 09 01 CC 00 00 00 10 47 02 00 01";

// Secure error response: count=0, return_code=0xFC.
const SECURE_READ_PID47_DENIED: &str = "3C 60 #BDUT_ADDR #EDI 0A 01 CD 00 00 00 10 47 00 00 01 FC";

// Secure success response: count=2, data = 4 bytes (wildcard).
// APDU: 14 bytes → extended frame len = 0x0D
const SECURE_READ_PID47_OK: &str = "3C 60 #BDUT_ADDR #EDI 0D 01 CD 00 00 00 10 47 02 00 01 00 00 ?? ??";
...
// → TP1 len = 0x6D (standard frame: 0x60 | 13)
const PLAIN_WRITE_PID47: &str = "BC #EDI #BDUT_ADDR 6D 01 CE 00 00 00 10 47 02 00 01 11 11 11 11";
...
const PLAIN_WRITE_PID47_RO: &str = "BC #BDUT_ADDR #EDI 6A 01 CF 00 00 00 10 47 00 00 01 FB";

// Secure WriteCon template.
const SECURE_WRITE_PID47: &str = "3C 60 #EDI #BDUT_ADDR 0D 01 CE 00 00 00 10 47 02 00 01 11 11 11 11";

// Secure write error response: return_code=0xFB.
const SECURE_WRITE_PID47_RO: &str = "3C 60 #BDUT_ADDR #EDI 0A 01 CF 00 00 00 10 47 00 00 01 FB";

// ============================================================================
// PropertyExtDescription_Read / Response templates for PID 0x47 on Device Object
// ============================================================================

// Plain A_PropertyExtDescription_Read (0x01D2): IOT=0x0000, instance=0x0010,
// PID=0x47 (IO_LIST), description index=0x00, property index=0x00.
const PLAIN_DESC_READ_PID47: &str = "BC #EDI #BDUT_ADDR 68 01 D2 00 00 00 10 47 00 00";

// Plain error response: all-zero descriptor (access denied, no error code — just zeroed).
const PLAIN_DESC_READ_PID47_DENIED: &str =
    "3C 60 #BDUT_ADDR #EDI 10 01 D3 00 00 00 10 47 00 00 00 00 00 00 00 00 00 00";

// Plain success response: valid descriptor (wildcard data bytes).
const PLAIN_DESC_READ_PID47_OK: &str = "3C 60 #BDUT_ADDR #EDI 10 01 D3 00 00 00 10 47 ?? ?? ?? ?? ?? ?? ?? ?? ?? ??";
...
pub fn create_section_3_8_6_suite() -> TestSuite {
    ...
    TestSuite::new("3.8.6 PID_IO_LIST (Device Object, access 3FF/0CC) [optional-recommended]", variables)
        .secure()
        .with_cases(vec![test_3_8_6_1(), test_3_8_6_2(), test_3_8_6_3()])
}
...
// ============================================================================
// 3.8.6.3 PropertyDescriptionRead plain
// ============================================================================

fn test_3_8_6_3() -> TestCase {
    TestCase::new("3.8.6.3 PropertyDescriptionRead plain").with_steps(vec![
        // ==== Security Mode ON ====
        comment("Enable Security Mode"),
        inject_secure_ac(ENABLE_SECURITY_MODE, "TK1"),
        expect_secure_ac(ENABLE_SECURITY_MODE_RESP, "TK1", TIMEOUT),
        comment("Plain description read → all-zero response (access denied)"),
        inject(PLAIN_DESC_READ_PID47),
        expect(PLAIN_DESC_READ_PID47_DENIED, TIMEOUT),
        // ==== Security Mode OFF ====
        comment("Disable Security Mode"),
        inject_secure_ac(DISABLE_SECURITY_MODE, "TK1"),
        expect_secure_ac(DISABLE_SECURITY_MODE_RESP, "TK1", TIMEOUT),
        ...
        inject(PLAIN_DESC_READ_PID47),
        expect(PLAIN_DESC_READ_PID47_OK, TIMEOUT),
    ])
}
