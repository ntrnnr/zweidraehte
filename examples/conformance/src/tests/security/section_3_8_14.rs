...
const SECURE_READ_CTRL: &str = "3C 60 #EDI #BDUT_ADDR 09 01 CC 00 11 00 10 3A 01 00 01";

// Secure read success: count=1, start=1, data=1 byte (wildcard).
// APDU: 01 CD + 00 11 + 00 10 + 3A + 01 + 00 01 + ?? = 11 bytes → len = 0x0A
#[allow(dead_code)]
const SECURE_READ_CTRL_OK: &str = "3C 60 #BDUT_ADDR #EDI 0A 01 CD 00 11 00 10 3A 01 00 01 ??";

// Secure read denied: count=0, start=1, return_code=0xFC.
const SECURE_READ_CTRL_DENIED: &str = "3C 60 #BDUT_ADDR #EDI 0A 01 CD 00 11 00 10 3A 00 00 01 FC";

// Secure A+C write: count=1, start=1, data=0x00.
// APDU: 01 CE + 00 11 + 00 10 + 3A + 01 + 00 01 + 00 = 11 bytes → len = 0x0A
const SECURE_WRITE_CTRL: &str = "3C 60 #EDI #BDUT_ADDR 0A 01 CE 00 11 00 10 3A 01 00 01 00";

// Secure write success: count=1, start=1, return_code=0x00.
#[allow(dead_code)]
const SECURE_WRITE_CTRL_OK: &str = "3C 60 #BDUT_ADDR #EDI 0A 01 CF 00 11 00 10 3A 01 00 01 00";

// Secure write denied: count=0, start=1, return_code=0xFC.
const SECURE_WRITE_CTRL_DENIED: &str = "3C 60 #BDUT_ADDR #EDI 0A 01 CF 00 11 00 10 3A 00 00 01 FC";

// Plain read: count=1, start=1.
// APDU: 10 bytes → TP1 len = 0x69
const PLAIN_READ_CTRL: &str = "BC #EDI #BDUT_ADDR 69 01 CC 00 11 00 10 3A 01 00 01";

// Plain read denied.
const PLAIN_READ_CTRL_DENIED: &str = "BC #BDUT_ADDR #EDI 6A 01 CD 00 11 00 10 3A 00 00 01 FC";

// Plain write: count=1, start=1, data=0x00.
// APDU: 11 bytes → TP1 len = 0x6A
const PLAIN_WRITE_CTRL: &str = "BC #EDI #BDUT_ADDR 6A 01 CE 00 11 00 10 3A 01 00 01 00";

// Plain write denied.
const PLAIN_WRITE_CTRL_DENIED: &str = "BC #BDUT_ADDR #EDI 6A 01 CF 00 11 00 10 3A 00 00 01 FC";
...
// Secure A+C read: count=1, start=1.
const SECURE_READ_RPT: &str = "3C 60 #EDI #BDUT_ADDR 09 01 CC 00 11 00 10 39 01 00 01";

// Secure read success: count=1, start=1, data=1 byte (wildcard).
const SECURE_READ_RPT_OK: &str = "3C 60 #BDUT_ADDR #EDI 0A 01 CD 00 11 00 10 39 01 00 01 ??";

// Secure read denied: count=0, start=1, return_code=0xFC.
const SECURE_READ_RPT_DENIED: &str = "3C 60 #BDUT_ADDR #EDI 0A 01 CD 00 11 00 10 39 00 00 01 FC";

// Secure A+C write: count=1, start=1, data=0x00.
const SECURE_WRITE_RPT: &str = "3C 60 #EDI #BDUT_ADDR 0A 01 CE 00 11 00 10 39 01 00 01 00";

// Secure write success.
const SECURE_WRITE_RPT_OK: &str = "3C 60 #BDUT_ADDR #EDI 0A 01 CF 00 11 00 10 39 01 00 01 00";

// Secure write denied.
const SECURE_WRITE_RPT_DENIED: &str = "3C 60 #BDUT_ADDR #EDI 0A 01 CF 00 11 00 10 39 00 00 01 FC";

// Plain read: count=1, start=1.
const PLAIN_READ_RPT: &str = "BC #EDI #BDUT_ADDR 69 01 CC 00 11 00 10 39 01 00 01";

// Plain read denied (sec ON). Note: 1FF policy allows plain read when sec OFF.
const PLAIN_READ_RPT_DENIED: &str = "BC #BDUT_ADDR #EDI 6A 01 CD 00 11 00 10 39 00 00 01 FC";

// Plain read success (sec OFF, 1FF policy allows plain read).
// APDU: 01 CD + 00 11 + 00 10 + 39 + 01 + 00 01 + ?? = 11 bytes → TP1 len = 0x6A
const PLAIN_READ_RPT_OK: &str = "BC #BDUT_ADDR #EDI 6A 01 CD 00 11 00 10 39 01 00 01 ??";

// Plain write: count=1, start=1, data=0x00.
const PLAIN_WRITE_RPT: &str = "BC #EDI #BDUT_ADDR 6A 01 CE 00 11 00 10 39 01 00 01 00";

// Plain write denied (0CC: write always requires A+C).
const PLAIN_WRITE_RPT_DENIED: &str = "BC #BDUT_ADDR #EDI 6A 01 CF 00 11 00 10 39 00 00 01 FC";
...
const PLAIN_DESC_READ_CTRL: &str = "BC #EDI #BDUT_ADDR 68 01 D2 00 11 00 10 3A 00 00";

// All-zero descriptor (access denied — 00C/00C plain never allowed).
const PLAIN_DESC_READ_CTRL_ZERO: &str = "3C 60 #BDUT_ADDR #EDI 10 01 D3 00 11 00 10 3A 00 00 00 00 00 00 00 00 00 00";

// Plain description read for PID_SECURITY_REPORT (0x39).
const PLAIN_DESC_READ_RPT: &str = "BC #EDI #BDUT_ADDR 68 01 D2 00 11 00 10 39 00 00";

// All-zero descriptor (sec ON, 1FF policy denies plain).
const PLAIN_DESC_READ_RPT_DENIED: &str = "3C 60 #BDUT_ADDR #EDI 10 01 D3 00 11 00 10 39 00 00 00 00 00 00 00 00 00 00";

// Valid descriptor (sec OFF, 1FF policy allows plain).
const PLAIN_DESC_READ_RPT_OK: &str = "3C 60 #BDUT_ADDR #EDI 10 01 D3 00 11 00 10 39 ?? ?? ?? ?? ?? ?? ?? ?? ?? ??";

// Secure A+C description read for PID_SECURITY_REPORT_CONTROL (0x3A).
// APDU: 01 D2 + 00 11 + 00 10 + 3A + 00 + 00 = 8 bytes → len = 0x08
const SECURE_DESC_READ_CTRL: &str = "3C 60 #EDI #BDUT_ADDR 08 01 D2 00 11 00 10 3A 00 00";

// Secure A+C success response: valid descriptor (wildcard data bytes).
const SECURE_DESC_READ_CTRL_OK: &str = "3C 60 #BDUT_ADDR #EDI 10 01 D3 00 11 00 10 3A ?? ?? ?? ?? ?? ?? ?? ?? ?? ??";
...
pub fn create_section_3_8_14_suite() -> TestSuite {
    ...
    TestSuite::new("3.8.14 PID_SECURITY_REPORT / REPORT_CONTROL (Security IO)", variables).secure().with_cases(vec![
        test_3_8_14_1(),
        test_3_8_14_2(),
        test_3_8_14_3(),
        test_3_8_14_4(),
