...
fn test_4_5_1() -> TestCase {
    TestCase::new("4.5.1 existing property description").with_steps(vec![
        ...
        expect("3C 60 #BDUT_ADDR #EDI 10 01 D3 00 00 00 10 0B ?? ?? ?? ?? ?? ?? ?? ?? ?? ??", TIMEOUT),
        comment("Read description of PID 0x0E (DEVICE_CONTROL) on Device"),
        inject("BC #EDI #BDUT_ADDR 68 01 D2 00 00 00 10 0E 00 00"),
        expect("3C 60 #BDUT_ADDR #EDI 10 01 D3 00 00 00 10 0E ?? ?? ?? ?? ?? ?? ?? ?? ?? ??", TIMEOUT),
    ])
}
...
fn test_4_5_2() -> TestCase {
    TestCase::new("4.5.2 non-existing IO type").with_steps(vec![
        ...
        expect("3C 60 #BDUT_ADDR #EDI 10 01 D3 00 0F 00 10 0B 00 00 00 00 00 00 00 00 00 00", TIMEOUT),
        comment("IOT 0x8000 → error response (zeros)"),
        inject("BC #EDI #BDUT_ADDR 68 01 D2 80 00 00 10 0B 00 00"),
        expect("3C 60 #BDUT_ADDR #EDI 10 01 D3 80 00 00 10 0B 00 00 00 00 00 00 00 00 00 00", TIMEOUT),
    ])
}
...
fn test_4_5_3() -> TestCase {
    TestCase::new("4.5.3 non-existing IO instance").with_steps(vec![
        ...
        expect("3C 60 #BDUT_ADDR #EDI 10 01 D3 00 00 00 00 0B 00 00 00 00 00 00 00 00 00 00", TIMEOUT),
        comment("Instance 0x0020 → error"),
        inject("BC #EDI #BDUT_ADDR 68 01 D2 00 00 00 20 0B 00 00"),
        expect("3C 60 #BDUT_ADDR #EDI 10 01 D3 00 00 00 20 0B 00 00 00 00 00 00 00 00 00 00", TIMEOUT),
        comment("Instance 0x8000 → error"),
        inject("BC #EDI #BDUT_ADDR 68 01 D2 00 00 80 00 0B 00 00"),
        expect("3C 60 #BDUT_ADDR #EDI 10 01 D3 00 00 80 00 0B 00 00 00 00 00 00 00 00 00 00", TIMEOUT),
    ])
}
...
fn test_4_5_5() -> TestCase {
    TestCase::new("4.5.5 non-existing property index").with_steps(vec![
        ...
        expect("3C 60 #BDUT_ADDR #EDI 10 01 D3 00 00 00 10 00 00 FF 00 00 00 00 00 00 00 00", TIMEOUT),
        comment("PropIdx 0x0800 (high byte in desc_type field) → error"),
        inject("BC #EDI #BDUT_ADDR 68 01 D2 00 00 00 10 00 08 00"),
        expect("3C 60 #BDUT_ADDR #EDI 10 01 D3 00 00 00 10 00 08 00 00 00 00 00 00 00 00 00", TIMEOUT),
    ])
}
...
fn test_4_5_7() -> TestCase {
    TestCase::new("4.5.7 non-existing PID").with_steps(vec![
        ...
        expect("3C 60 #BDUT_ADDR #EDI 10 01 D3 00 00 00 10 FF 00 00 00 00 00 00 00 00 00 00", TIMEOUT),
        comment("PID 0 on non-existing instance 0x0018 → error"),
        inject("BC #EDI #BDUT_ADDR 68 01 D2 00 00 00 18 00 00 00"),
        expect("3C 60 #BDUT_ADDR #EDI 10 01 D3 00 00 00 18 00 00 00 00 00 00 00 00 00 00 00", TIMEOUT),
    ])
}
...
fn test_4_5_4() -> TestCase {
    TestCase::new("4.5.4 description read by property index").with_steps(vec![
        ...
        expect("3C 60 #BDUT_ADDR #EDI 10 01 D3 00 00 00 10 0B 00 #INDX_PID_SERIAL_NO ?? ?? ?? ?? ?? ?? ?? ??", TIMEOUT),
