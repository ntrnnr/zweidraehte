...
const CLEAR_LOG: &str = "3C 60 #EDI #BDUT_ADDR 09 01 D4 00 11 00 10 37 00 00 00";

// Response to clear: rc=00, data=[id=00].
const CLEAR_LOG_RESP: &str = "3C 60 #BDUT_ADDR #EDI 08 01 D6 00 11 00 10 37 00 00";

// FunctionPropertyExtStateRead: read all counters (id=0, info=0).
const READ_COUNTERS: &str = "3C 60 #EDI #BDUT_ADDR 09 01 D5 00 11 00 10 37 00 00 00";
...
const READ_LAST_ENTRY: &str = "3C 60 #EDI #BDUT_ADDR 09 01 D5 00 11 00 10 37 00 01 00";
...
const PROVOKE_CRYPTO: &str = "3C 60 #EDI #BDUT_ADDR 08 01 D5 00 11 00 10 33 00 00";
...
const PROVOKE_ACCESS: &str = "3C 60 #EDI #BDUT_ADDR 09 01 CC 00 11 00 10 3B 01 00 00";

// Response to access provocation: PropertyExtValueWriteConRes with error FC.
const PROVOKE_ACCESS_RESP: &str = "3C 60 #BDUT_ADDR #EDI 0A 01 CD 00 11 00 10 3B 00 00 00 FC";

// Provoke SeqNrError: FunctionPropertyExtStateRead on PID 0x37 with seq=0.
// DUT rejects (seq=0 invalid) → drops, no response.
const PROVOKE_SEQ: &str = "3C 60 #EDI #BDUT_ADDR 09 01 D5 00 11 00 10 37 00 00 00";
...
const NEG_BAD_ID: &str = "3C 60 #EDI #BDUT_ADDR 09 01 D5 00 11 00 10 37 00 05 00";
const NEG_BAD_ID_RESP: &str = "3C 60 #BDUT_ADDR #EDI 08 01 D6 00 11 00 10 37 F2 05";

// StateRead with wrong service info (id=0, info=0x11) → F8, data=[00].
const NEG_BAD_INFO: &str = "3C 60 #EDI #BDUT_ADDR 09 01 D5 00 11 00 10 37 00 00 11";
const NEG_BAD_INFO_RESP: &str = "3C 60 #BDUT_ADDR #EDI 08 01 D6 00 11 00 10 37 F8 00";

// StateRead with out-of-bounds entry index (id=1, info=8) → F8, data=[01].
const NEG_OOB_ENTRY: &str = "3C 60 #EDI #BDUT_ADDR 09 01 D5 00 11 00 10 37 00 01 08";
const NEG_OOB_ENTRY_RESP: &str = "3C 60 #BDUT_ADDR #EDI 08 01 D6 00 11 00 10 37 F8 01";

// StateRead with incorrect length (only id, no info byte) → F8, data=[00].
const NEG_SHORT: &str = "3C 60 #EDI #BDUT_ADDR 07 01 D5 00 11 00 10 37 00";
const NEG_SHORT_RESP: &str = "3C 60 #BDUT_ADDR #EDI 08 01 D6 00 11 00 10 37 F8 00";

// Unsecured FctPropStateRead on PID 0x37 → FC (denied, SM=ON).
const UNSECURED_STATE_READ: &str = "BC #EDI #BDUT_ADDR 69 01 D5 00 11 00 10 37 00 00 00";
const UNSECURED_STATE_READ_RESP: &str = "BC #BDUT_ADDR #EDI 68 01 D6 00 11 00 10 37 FC 00";

// Unsecured read of last entry → FC.
const UNSECURED_LAST_ENTRY: &str = "BC #EDI #BDUT_ADDR 69 01 D5 00 11 00 10 37 00 01 00";
const UNSECURED_LAST_ENTRY_RESP: &str = "BC #BDUT_ADDR #EDI 68 01 D6 00 11 00 10 37 FC 01";

// Auth-only FctPropStateRead on PID 0x37 → FC (SM=ON, 1FF needs A+C).
const AUTH_ONLY_STATE_READ: &str = "3C 60 #EDI #BDUT_ADDR 09 01 D5 00 11 00 10 37 00 00 00";
const AUTH_ONLY_STATE_READ_RESP: &str = "3C 60 #BDUT_ADDR #EDI 08 01 D6 00 11 00 10 37 FC 00";

// Auth-only read of last entry → FC.
const AUTH_ONLY_LAST_ENTRY: &str = "3C 60 #EDI #BDUT_ADDR 09 01 D5 00 11 00 10 37 00 01 00";
const AUTH_ONLY_LAST_ENTRY_RESP: &str = "3C 60 #BDUT_ADDR #EDI 08 01 D6 00 11 00 10 37 FC 01";
...
const PLAIN_DESC_READ: &str = "BC #EDI #BDUT_ADDR 68 01 D2 00 11 00 10 37 00 00";

// Per XML: all-zero descriptor (sec ON, 1FF denies plain desc when sec ON).
const PLAIN_DESC_READ_DENIED: &str = "3C 60 #BDUT_ADDR #EDI 10 01 D3 00 11 00 10 37 00 00 00 00 00 00 00 00 00 00";

// Per XML: valid descriptor (sec OFF, 1FF allows plain).
const PLAIN_DESC_READ_OK: &str = "3C 60 #BDUT_ADDR #EDI 10 01 D3 00 11 00 10 37 ?? ?? ?? ?? ?? ?? ?? ?? ?? ??";
...
fn test_3_8_12_1() -> TestCase {
    ...
    TestCase::new("3.8.12.1 Secure FunctionProperty, behavior on Power Down").with_steps(steps)
}
...
fn test_3_8_12_2() -> TestCase {
    ...
    TestCase::new("3.8.12.2 Secure FunctionPropertyCommand, behavior on Confirmed Restart").with_steps(steps)
}
...
fn test_3_8_12_3() -> TestCase {
    ...
    const READ_COUNTERS_EMPTY: &str = "3C 60 #BDUT_ADDR #EDI 11 01 D6 00 11 00 10 37 00 00 00 00 00 00 00 00 00 00 00";
    // F8 = E_BAD_ARGUMENT — no entry at index 1 because the log is empty.
    const READ_LAST_ENTRY_EMPTY: &str = "3C 60 #BDUT_ADDR #EDI 08 01 D6 00 11 00 10 37 F8 01";
...
fn test_3_8_12_6() -> TestCase {
    ...
    TestCase::new("3.8.12.6 Check prevention of Overflow in security counters").with_steps(steps)
}
...
fn test_3_8_12_7() -> TestCase {
    ...
    TestCase::new("3.8.12.7 Secure FunctionPropertyStateRead, negative cases").with_steps(steps)
}
