...
const PLAIN_READ_PID0B: &str = "BC #EDI #BDUT_ADDR 69 01 CC 00 00 00 10 0B 01 00 01";

// Plain error response: count=0, return_code=0xFC (E_ACCESS_DENIED).
const PLAIN_READ_PID0B_DENIED: &str = "BC #BDUT_ADDR #EDI 6A 01 CD 00 00 00 10 0B 00 00 01 FC";

// Plain success response: count=1, data=6 bytes (serial number, wildcard).
const PLAIN_READ_PID0B_OK: &str = "BC #BDUT_ADDR #EDI 6F 01 CD 00 00 00 10 0B 01 00 01 ?? ?? ?? ?? ?? ??";

// Secure read template (same inner APDU, carried in extended frame for secure wrapping).
const SECURE_READ_PID0B: &str = "3C 60 #EDI #BDUT_ADDR 09 01 CC 00 00 00 10 0B 01 00 01";

// Secure error response: count=0, return_code=0xFC.
const SECURE_READ_PID0B_DENIED: &str = "3C 60 #BDUT_ADDR #EDI 0A 01 CD 00 00 00 10 0B 00 00 01 FC";

// Secure success response: count=1, data=6 bytes (serial number, wildcard).
const SECURE_READ_PID0B_OK: &str = "3C 60 #BDUT_ADDR #EDI 0F 01 CD 00 00 00 10 0B 01 00 01 ?? ?? ?? ?? ?? ??";
...
const PLAIN_WRITE_PID0B: &str = "BC #EDI #BDUT_ADDR 6F 01 CE 00 00 00 10 0B 01 00 01 00 00 00 00 00 01";

// Plain write error response: count=0, return_code=0xFB (E_ACCESS_READ_ONLY).
const PLAIN_WRITE_PID0B_RO: &str = "BC #BDUT_ADDR #EDI 6A 01 CF 00 00 00 10 0B 00 00 01 FB";

// Secure WriteCon template.
const SECURE_WRITE_PID0B: &str = "3C 60 #EDI #BDUT_ADDR 0F 01 CE 00 00 00 10 0B 01 00 01 00 00 00 00 00 01";

// Secure write error response: return_code=0xFB.
const SECURE_WRITE_PID0B_RO: &str = "3C 60 #BDUT_ADDR #EDI 0A 01 CF 00 00 00 10 0B 00 00 01 FB";
...
const PLAIN_DESC_READ_PID0B: &str = "BC #EDI #BDUT_ADDR 68 01 D2 00 00 00 10 0B 00 00";
...
const PLAIN_DESC_READ_PID0B_OK: &str = "3C 60 #BDUT_ADDR #EDI 10 01 D3 00 00 00 10 0B ?? ?? ?? ?? ?? ?? ?? ?? ?? ??";
...
pub fn create_section_3_8_3_suite() -> TestSuite {
    ...
    TestSuite::new("3.8.3 PID_SERIAL_NUMBER (Device Object, access 3FF/0CC)", variables).secure().with_cases(vec![
        test_3_8_3_1(),
        test_3_8_3_2(),
        test_3_8_3_3(),
    ])
