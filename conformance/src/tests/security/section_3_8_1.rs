...
const PLAIN_READ_PID1: &str = "BC #EDI #BDUT_ADDR 69 01 CC 00 11 00 10 01 01 00 01";

// Plain error response: count=0, return_code=0xFC (E_ACCESS_DENIED).
const PLAIN_READ_PID1_DENIED: &str = "BC #BDUT_ADDR #EDI 6A 01 CD 00 11 00 10 01 00 00 01 FC";

// Plain success response: count=1, data=0x0011 (Security object type).
const PLAIN_READ_PID1_OK: &str = "BC #BDUT_ADDR #EDI 6B 01 CD 00 11 00 10 01 01 00 01 00 11";

// Secure read template (same inner APDU, carried in extended frame for secure wrapping).
const SECURE_READ_PID1: &str = "3C 60 #EDI #BDUT_ADDR 09 01 CC 00 11 00 10 01 01 00 01";

// Secure error response: count=0, return_code=0xFC.
const SECURE_READ_PID1_DENIED: &str = "3C 60 #BDUT_ADDR #EDI 0A 01 CD 00 11 00 10 01 00 00 01 FC";

// Secure success response: count=1, data=0x0011.
const SECURE_READ_PID1_OK: &str = "3C 60 #BDUT_ADDR #EDI 0B 01 CD 00 11 00 10 01 01 00 01 00 11";
...
const PLAIN_WRITE_PID1: &str = "BC #EDI #BDUT_ADDR 6A 01 CE 00 11 00 10 01 01 00 01 00 18";

// Plain write error response: count=0, return_code=0xFB (E_ACCESS_READ_ONLY).
const PLAIN_WRITE_PID1_RO: &str = "BC #BDUT_ADDR #EDI 6A 01 CF 00 11 00 10 01 00 00 01 FB";

// Secure WriteCon template.
const SECURE_WRITE_PID1: &str = "3C 60 #EDI #BDUT_ADDR 0A 01 CE 00 11 00 10 01 01 00 01 00 18";

// Secure write error response: return_code=0xFB.
const SECURE_WRITE_PID1_RO: &str = "3C 60 #BDUT_ADDR #EDI 0A 01 CF 00 11 00 10 01 00 00 01 FB";
...
const PLAIN_DESC_READ_PID1: &str = "BC #EDI #BDUT_ADDR 68 01 D2 00 11 00 10 01 00 00";
...
const PLAIN_DESC_READ_PID1_DENIED: &str = "3C 60 #BDUT_ADDR #EDI 10 01 D3 00 11 00 10 01 00 00 00 00 00 00 00 00 00 00";

// Plain success response: valid descriptor (wildcard data bytes).
const PLAIN_DESC_READ_PID1_OK: &str = "3C 60 #BDUT_ADDR #EDI 10 01 D3 00 11 00 10 01 ?? ?? ?? ?? ?? ?? ?? ?? ?? ??";
...
pub fn create_section_3_8_1_suite() -> TestSuite {
    ...
    TestSuite::new("3.8.1 PID_OBJECT_TYPE (Security IO, access 3FF/0CC)", variables).secure().with_cases(vec![
        test_3_8_1_1(),
        test_3_8_1_2(),
        test_3_8_1_3(),
    ])
