...
|--------|-----------|---------|
| **Type** | Simple enum (`#[derive(EtsEnum)]`) | Discriminated union (`#[ets_union]`) |
...
Use `#[ets_union]` for parameters that have different fields depending on a mode selector.

It is an **attribute macro**, not a derive, because it rewrites the enum: it inserts the
alignment and tail padding a tagged union needs so that every byte of the union is
initialised, sets `#[repr(u8)]`, and applies `#[derive(zerocopy::IntoBytes)]` — the check
that proves there is no uninitialized padding. That matters because a union's bytes are
read wholesale, both as the `<Data>` defaults blob ETS reads back and as the live
parameter memory served to `A_Memory_Read`.

Consequences for how you write one:

- **Do not add a `repr`.** The macro sets it. (`#[repr(C, u8)]` in particular blocks
  zerocopy's derive, which is why it is rejected.)
- **Never hand-write `unsafe impl IntoBytes`.** There is no case where it is needed.
- **Unit variants gain a body**, since they must still span the union's full width:
  `ButtonConfig::Dimmer` is written `ButtonConfig::Dimmer { .. }` in patterns. Mark the
  default variant with `#[ets(default_variant)]` and the macro generates `ConstDefault`
  for you, so generated padding fields never have to be named at a construction site.
- **Variants must use named fields** (or none); tuple variants are rejected.
...
Unions (`#[ets_union]`) require matching on the discriminant:
